...
public class PruebaFicherosJava {	
		...
		public static void mostrarMenu() {
			...
			System.out.println("  3) Renombra un File usando ruta + nombre");
			System.out.println("  4) Borra un File usando ruta + nombre");
			System.out.print("OpciÃ³n seleccionada (0 para finalizar): ");
		}
		
		
		public static void renombrarFichero (File f, File f2) {
			f.renameTo (f2);
		}
		
		public static void borrarFichero (File f) {
			f.delete ();
		}
			...
			while(opcion != 0) {
				...
				if(opcion == 1 || opcion == 2 || opcion == 3 || opcion == 4) {
					if(opcion == 1) {
						...
						System.out.print("Escriba el nombre la ruta para File (sin nombre): ");
						String ruta = entrada.next();
						System.out.print("Escriba el nombre para File: ");
						String nombre = entrada.next();
						...
						renombrarFichero (f, f2);
					} else if ( opcion == 4) {
						System.out.print("Escriba el nuevo nombre la ruta para File (sin nombre): ");
						String ruta = entrada.next();
						System.out.print("Escriba el nombre para File: ");
						String nombre = entrada.next();
						f = new File(ruta, nombre);
